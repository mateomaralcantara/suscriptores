...
-- Backfill existing Supabase Auth users, then remove the temporary helper.
do $$
declare
  r record;
begin
...
drop function if exists public.handle_new_auth_user_backfill(uuid, text, jsonb);
...
declare
  ...
  v_wallet wallets%rowtype;
begin
  if nullif(trim(p_provider_order_id), '') is null then raise exception 'provider_order_id_required'; end if;
  ...
  values (
    ...
    v_order.cost_minor,
    'order',
    jsonb_build_object('public_id', v_order.public_id)
  )
...
begin
  ...
  set available_minor = available_minor + v_order.cost_minor,
      reserved_minor = reserved_minor - v_order.cost_minor
  where id = v_wallet.id;

  update orders
  set status = 'failed',
      metadata = coalesce(metadata, '{}'::jsonb) || jsonb_build_object('provider_error', left(coalesce(p_error, 'provider_error'), 1000)),
      updated_at = now()
  where id = v_order.id;

  insert into order_events(order_id, status, message)
  values (v_order.id, 'failed', left(coalesce(p_error, 'Provider error.'), 1000));
  return 'failed';
end;
$$;
...
begin
  ...
  set status = 'provider_unknown',
      metadata = coalesce(metadata, '{}'::jsonb) || jsonb_build_object('provider_error', left(coalesce(p_error, 'ambiguous_provider_response'), 1000)),
      updated_at = now()
  ...
  if v_status = 'provider_unknown' then
    insert into order_events(order_id, status, message)
    values (p_order_id, 'provider_unknown', left(coalesce(p_error, 'Provider response is ambiguous.'), 1000));
  else
